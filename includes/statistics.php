 ...
 */
function wrms_count_synced_posts(string $post_type, string $post_status = 'publish'): int
{
    global $wpdb;
 ...
 */
function wrms_count_synced_terms(string $taxonomy): int
{
    global $wpdb;
 ...
 */
function wrms_calculate_and_cache_stats(): array
{
    ...
    $sync_percentage = $total_items > 0 ? round(($total_synced / $total_items) * 100, 2) : 0.0;
 ...
 */
function wrms_get_stats(): array
{
    $stats = get_option('wrms_stats_cache');
    return $stats ?: wrms_calculate_and_cache_stats();
}
